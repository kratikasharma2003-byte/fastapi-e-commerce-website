...
def stripe_success(
    ...
    user_email = request.session.get("stripe_email")

    # Fallback: look up via session metadata if session cookie is lost
    if not user_email:
        try:
            session = stripe.checkout.Session.retrieve(session_id)
            user_email = session.customer_email or session.metadata.get("user_email")
        except Exception:
            return RedirectResponse(url="/stripe/cancel-page?reason=session_lost", status_code=303)
    ...
    if not order:
        print(f"[Stripe/success] ⚠️  Order #{order_id} not found")
    ...
    try:
        ...
        print(f"[Stripe/success] ❌ Session retrieve error: {e}")
        order.status = "Failed"
        db.commit()
    ...
    try:
        db.commit()
        db.refresh(order)
        print(f"[Stripe/success] ✅ Order #{order.id} → Paid | PI={payment_intent_id}")
    except SQLAlchemyError as e:
        db.rollback()
    ...
    try:
        ...
        send_order_confirmation(user_email, order.id, order.total, items_for_email)
        print(f"[Stripe/success] 📧 Email sent to {user_email}")
    except Exception as e:
        print(f"[Stripe/success] ⚠️  Email failed (non-fatal): {e}")
